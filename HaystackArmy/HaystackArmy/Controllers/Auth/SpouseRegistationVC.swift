...
class SpouseRegistationVC: UIViewController {
    @IBOutlet weak var headerVewref: UIView!
    
    @IBOutlet weak var FNametfref: UITextField!
    @IBOutlet weak var LnameTfref: UITextField!
    @IBOutlet weak var PersonalEmailTfref: UITextField!
    @IBOutlet weak var RelationShipTfref: UITextField!
...
extension SpouseRegistationVC: UITextFieldDelegate{
    ...
    func RegistrationValidation()-> ApiCheckValidation {
        if FNametfref.text ?? "" == "" || LnameTfref.text ?? "" == "" || PersonalEmailTfref.text ?? "" == "" || RelationShipTfref.text ?? "" == "" || GovtEmailAddressTfref.text ?? "" == "" || UserNameTfref.text ?? "" == "" || PasswordTfref.text ?? "" == "" || ConfirmPasswordTfref.text ?? "" == "" {
...
extension SpouseRegistationVC {
    ...
    func Registration(){
        ...
        guard let Lname = LnameTfref.text else{
            indicator.hideActivityIndicator()
            return}
        guard let RelationShip = RelationShipTfref.text else{
            indicator.hideActivityIndicator()
            return}
        guard let email = PersonalEmailTfref.text else{
            indicator.hideActivityIndicator()
            return}
        guard let Govtemail = GovtEmailAddressTfref.text else{
            indicator.hideActivityIndicator()
            return}
        ...
        let parameters = [
            ...
            "lname":Lname,
            "email":email,
            "sponsors_email":Govtemail,
            "username":UserName,
            "password":password,
            "relation_to_sm":RelationShip,
            "device_type":"IOS",
            "device_id":UIDevice.current.identifierForVendor!.uuidString,
            "device_token":"3543546854564685451"
        ]
        NetworkManager.Apicalling(url: API_URl.SpouseSignUPURL, paramaters: parameters, httpMethodType: .post, success: { (response:RegisterUserModel) in
            print(response.uid)
