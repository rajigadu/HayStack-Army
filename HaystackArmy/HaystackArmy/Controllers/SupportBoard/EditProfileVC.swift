    ...
    override func viewDidLoad() {
       ...
       // self.headerVewref.addBottomShadow()
        ...
        if UserDefaults.standard.string(forKey: "LoginedUserType") == "Soldier"{
            ...
            if let FUserName = UserDefaults.standard.string(forKey: "Spousefname") {
                self.FNametfref.text = FUserName
            }
            if let LUserName = UserDefaults.standard.string(forKey: "Spouselname") {
                self.LNametfref.text = LUserName
            }
            if let UserName = UserDefaults.standard.string(forKey: "SpouseUserName") {
                self.UserNametfref.text = UserName
            }
        }
    ...
    func EditProfileMethod(){
        ...
        NetworkManager.Apicalling(url: API_URl.UpdateProfileURL, paramaters: parameters, httpMethodType: .post, success: { (response:RegisterUserModel) in
            ...
            if response.status == "1" {
                ...
                if UserDefaults.standard.string(forKey: "LoginedUserType") == "Soldier"{
                    UserDefaults.standard.set(FName, forKey: "Soldierfname")
                    UserDefaults.standard.set(LName, forKey: "Soldierlname")
                    UserDefaults.standard.set(UserName, forKey: "SoldierUsername")
                  }else {
                    UserDefaults.standard.set(FName, forKey: "Spousefname")
                    UserDefaults.standard.set(LName, forKey: "Spouselname")
                    UserDefaults.standard.set(UserName, forKey: "SpouseUserName")
                  }
